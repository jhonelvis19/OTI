...
<div class="max-w-4xl mx-auto py-8 px-4">
    ...
    <div class="flex items-center justify-between mb-8 pb-4 border-b border-indigo-100">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-800 tracking-tight">Mi Perfil</h1>
            <p class="text-sm text-gray-500 mt-1">Gestione su información de usuario y firma digital.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-sm font-semibold text-slate-800 mt-0.5">{{ auth()->user()->name }} {{ auth()->user()->apellido }}</p>
                    </div>
                    <div>
        ...
        <div class="md:col-span-2 space-y-6">
            ...
            <div id="panel-firma-estado" class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                ...
                @if(auth()->user()->firma)
                    ...
                    <div id="firma-actual-view" class="space-y-4">
                        <div class="border border-indigo-100 rounded-2xl p-4 bg-slate-50 flex justify-center items-center h-48 shadow-inner relative">
                ...
                @else
                    ...
                    <div id="firma-vacia-view" class="text-center py-8 space-y-4">
                        ...
                        <div>
                            <p class="text-sm font-medium text-slate-700">No ha registrado su firma en su perfil</p>
                            <p class="text-xs text-gray-400 mt-1">Debe registrar su firma permanente para poder utilizarla al firmar los informes técnicos.</p>
                        </div>
                        <button type="button" id="btn-registrar-firma-trigger" class="inline-flex items-center gap-1 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold px-4 py-2 rounded-xl shadow-sm transition duration-200">
                            ✍ Registrar Firma
                        </button>
                    </div>
                @endif
            ...
            <!-- PANEL REGISTRO / ACTUALIZACIÓN DE FIRMA -->
            <div id="panel-firma-formulario" class="hidden bg-white p-6 rounded-2xl shadow-sm border border-gray-200 space-y-6">
                ...
                <form id="form-firma-perfil" class="space-y-6">
                    ...
                    <div class="flex gap-4">
                        <button type="button" id="btn-metodo-dibujar" class="flex-1 py-3 px-4 rounded-xl border border-indigo-300 bg-indigo-600 text-white font-semibold transition duration-200 text-xs">
                            ✍ Dibujar en Pantalla
                        </button>
                        <button type="button" id="btn-metodo-foto" class="flex-1 py-3 px-4 rounded-xl border border-indigo-300 bg-slate-100 text-slate-700 font-semibold hover:bg-slate-200 transition duration-200 text-xs">
                            📷 Tomar Fotografía
                        </button>
                    </div>
                    ...
                    <div class="border-t border-indigo-50 pt-4 space-y-3">
                        ...
                        <input type="password" name="password_confirmacion" id="password_confirmacion" required placeholder="••••••••" class="w-full rounded-xl border border-indigo-300 bg-slate-50 px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-400 focus:border-transparent focus:bg-white outline-none transition duration-200">
                    </div>

                    <button type="submit" id="btn-guardar-firma" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-4 rounded-xl shadow-md hover:shadow-lg transition duration-200 text-sm">
                        Confirmar y Registrar Firma
                    </button>
                </form>
            </div>

            <!-- PANEL CONFIRMACIÓN ELIMINACIÓN -->
            <div id="panel-firma-eliminar" class="hidden bg-white p-6 rounded-2xl shadow-sm border border-red-200 space-y-6">
                ...
                <form id="form-eliminar-firma" class="space-y-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="password" name="password_confirmacion" id="password_eliminar" required placeholder="••••••••" class="w-full rounded-xl border border-red-300 bg-slate-50 px-4 py-3 text-sm focus:ring-2 focus:ring-red-400 focus:border-transparent focus:bg-white outline-none transition duration-200">
                    </div>

                    <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-4 rounded-xl shadow-md hover:shadow-lg transition duration-200 text-sm">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    btnCancelarFormulario.addEventListener('click', function () {
        ...
        panelEstado.classList.remove('remove');
        window.location.reload(); // Recarga simple para resetear todo el estado
    });
    ...
    inputFoto.addEventListener('change', function (e) {
        ...
        if (file) {
            ...
            reader.onload = function (event) {
                ...
                inputBase64.value = event.target.result; // base64 de la foto
            };
    ...
    formFirma.addEventListener('submit', function (e) {
        ...
        // Si es dibujada o es foto cargada por FileReader, enviamos la cadena Base64
        formData.append('firma_base64', inputBase64.value);

        fetch('/usuario/perfil/firma', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        .catch(error => {
            ...
            showAlert('Ocurrió un error de red o de servidor al procesar la solicitud.', 'error');
        });
    });

    // Eliminar Firma (Submit AJAX)
